...
const juminzeiLocalTax: AdminGuide = {
    ...
    legalScope: {
      ...
      riskLevel: 'medium',
      whenToAskExpert: [
        'Thu nhập nhiều nguồn (lương + part-time + tự kinh doanh + remittance từ VN) → hỏi 税務署 hoặc 税理士 cách khai 確定申告 tránh bị truy thu.',
        'Bị truy thu 住民税 nhiều năm cũ + tiền phạt → hỏi 市役所 và có thể 税理士 về phương án chia kỳ / xin giảm.',
        'Đang ở tình trạng thu nhập đột giảm / mất việc / khó khăn tài chính → hỏi 市民税課 về 減免 sớm trước hạn (thường tháng 7–8).',
        'Sắp về VN nhưng còn 住民税 chưa nộp xong → hỏi 市役所 về 納税管理人 trước khi xuất cảnh, không tự bỏ.',
      ],
    },
    quickAction: {
      deadline: 'Hạn 普通徴収: 4 đợt mỗi năm (tháng 6, 8, 10, 1) — nộp tại konbini/ngân hàng/Pay-easy. 特別徴収 (qua lương): không cần làm gì, công ty tự trừ. 減免 申請: thường trước tháng 7–8, khác theo 市役所.',
      office: '市役所/区役所 phòng 市民税課 / 住民税担当. Online qua マイナポータル nếu địa phương có hỗ trợ.',
      doNow: [
        'Mở 給与明細 (phiếu lương) gần nhất, tìm dòng "住民税" — có dòng = 特別徴収 (qua lương); không có = 普通徴収 (tự nộp, đợi hoá đơn về nhà).',
        'Nhận 住民税決定通知書 tháng 6 → đọc 課税標準額 + 税額 → so với năm trước.',
        'Nếu thu nhập giảm / thất nghiệp → đến 市民税課 hỏi 減免 申請 trước hạn (thường 7–8).',
        'Nếu chuyển việc → nói với công ty cũ "特別徴収を続けたい" để chuyển sang công ty mới qua 異動届.',
        'Nếu sắp về VN còn nợ 住民税 → 市役所 đăng ký 納税管理人.',
      ],
      bring: [
        '在留カード',
        '住民税決定通知書 hoặc 納付書 (nếu có)',
        '給与明細 6 tháng gần nhất',
        '源泉徴収票 năm trước (cho mọi nguồn thu nhập)',
        '離職票 hoặc 雇用保険受給資格者証 (nếu xin 減免)',
        'Sổ ngân hàng / cash card (cho 口座振替 tự động)',
      ],
    ...
    keyTerms: [
      ...
      },
      {
        term: '均等割',
        reading: 'きんとうわり',
        meaningVi: 'phần thuế "đầu người" (cố định, không phụ thuộc thu nhập)',
        noteVi: 'Khoản cố định mỗi cư dân phải đóng — thường ~5,000円/năm (chia ~3,500円 cho tỉnh + ~1,500円 cho quận; thay đổi theo địa phương). KHÔNG miễn được trừ khi đặc biệt nghèo. Bạn thấy số này trên 住民税決定通知書 ghi rõ 均等割: XXX円.',
      },
      {
        term: '所得割',
        reading: 'しょとくわり',
        meaningVi: 'phần thuế tính theo thu nhập (~10% thu nhập chịu thuế)',
        noteVi: 'Phần chính của 住民税. Tính = (thu nhập năm trước - khoản miễn trừ) × 10% (6% tỉnh + 4% quận). Càng thu nhập cao càng đóng nhiều. Trên 住民税決定通知書 ghi 所得割: XXX円.',
      },
      {
        term: '異動届',
        reading: 'いどうとどけ',
        meaningVi: 'giấy báo thay đổi (chuyển việc / chuyển nhà liên quan thuế)',
        noteVi: 'Khi chuyển công ty mà muốn tiếp tục 特別徴収, công ty cũ làm 給与所得者異動届出書 gửi sang 市役所 + công ty mới. Tránh bị chuyển bất ngờ sang 普通徴収.',
      },
      {
        term: '住民税決定通知書',
        reading: 'じゅうみんぜいけっていつうちしょ',
        meaningVi: 'thông báo quyết định 住民税 (nhận tháng 6)',
        noteVi: 'Tài liệu chính thức ghi đầy đủ: 課税標準額 (thu nhập chịu thuế), 所得割, 均等割, tổng 税額, lịch nộp. Công ty đưa nếu bạn 特別徴収; 市役所 gửi qua bưu điện nếu bạn 普通徴収. GIỮ KỸ — cần cho thủ tục xin visa, vay tiền, thuê nhà.',
      },
    ],
    officialLinks: [
